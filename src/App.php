...
final class App
{
    ...
    public function __construct(string $root)
    {
        ...
        $this->db = new PDO($dsn, (string)($this->config['database_user'] ?? ''), (string)($this->config['database_password'] ?? ''), [PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION,PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC,PDO::ATTR_EMULATE_PREPARES=>false]);
        $this->migrate();
    }
    ...
    {
        $this->db->exec(<<<'SQL'
CREATE TABLE IF NOT EXISTS users (id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,name VARCHAR(160) NOT NULL,email VARCHAR(254) NOT NULL UNIQUE,password_hash VARCHAR(255) NULL,google_sub VARCHAR(255) NULL UNIQUE,avatar_url VARCHAR(1000) NULL,active TINYINT(1) NOT NULL DEFAULT 1,platform_role VARCHAR(32) NOT NULL DEFAULT 'user',points BIGINT NOT NULL DEFAULT 0,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS routers (id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,name VARCHAR(160) NOT NULL,identity VARCHAR(160) NOT NULL UNIQUE,public_host VARCHAR(255),location VARCHAR(255),api_key CHAR(48) NOT NULL UNIQUE,enabled TINYINT(1) NOT NULL DEFAULT 1,last_seen_at DATETIME,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
CREATE TABLE IF NOT EXISTS vendos (id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,owner_user_id BIGINT UNSIGNED NULL,router_id BIGINT UNSIGNED NOT NULL,name VARCHAR(160) NOT NULL,base_url VARCHAR(255) NOT NULL,server_ip VARCHAR(45) NULL,client_subnet VARCHAR(64) NULL,interface_name VARCHAR(128) NULL,password_mode VARCHAR(32) NOT NULL DEFAULT 'blank',charging_enabled TINYINT(1) NOT NULL DEFAULT 0,eload_enabled TINYINT(1) NOT NULL DEFAULT 0,enabled TINYINT(1) NOT NULL DEFAULT 1,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,INDEX idx_vendos_owner (owner_user_id),INDEX idx_vendos_router (router_id),FOREIGN KEY(owner_user_id) REFERENCES users(id) ON DELETE SET NULL,FOREIGN KEY(router_id) REFERENCES routers(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
SQL);
        ...
        $this->db->exec("INSERT IGNORE INTO vendo_settings(setting_key,setting_value) VALUES('hotspot_debug','0')");
        ...
        try { $this->db->exec("CREATE INDEX idx_vendos_server_ip ON vendos (server_ip)"); } catch (PDOException) {}
...
function require_csrf(): void { $provided=(string)($_POST['_csrf']??''); if(!hash_equals((string)($_SESSION['csrf']??''),$provided)){http_response_code(419);exit('The form expired. Go back and try again.');} }
function redirect(string $path): never { header('Location: '.$path,true,303); exit; }
function client_mac(string $value): string { $hex=strtoupper(preg_replace('/[^0-9A-Fa-f]/','',$value)); return strlen($hex)===12?implode(':',str_split($hex,2)):''; }
function bytes_nice(int $bytes): string { $units=['B','KB','MB','GB','TB'];$size=max(0,$bytes);$unit=0;while($size>=1024&&$unit<count($units)-1){$size/=1024;$unit++;}return number_format($size,$unit===0?0:1).' '.$units[$unit]; }
function duration_nice(int $seconds): string { $seconds=max(0,$seconds);$hours=intdiv($seconds,3600);$minutes=intdiv($seconds%3600,60);return $hours>0?"{$hours}h {$minutes}m":"{$minutes}m"; }
