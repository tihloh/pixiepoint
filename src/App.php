...
CREATE TABLE IF NOT EXISTS vendos (id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,owner_user_id BIGINT UNSIGNED NULL,router_id BIGINT UNSIGNED NOT NULL,name VARCHAR(160) NOT NULL,base_url VARCHAR(255) NOT NULL,client_subnet VARCHAR(64) NULL,interface_name VARCHAR(128) NULL,password_mode VARCHAR(32) NOT NULL DEFAULT 'blank',charging_enabled TINYINT(1) NOT NULL DEFAULT 0,eload_enabled TINYINT(1) NOT NULL DEFAULT 0,enabled TINYINT(1) NOT NULL DEFAULT 1,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,INDEX idx_vendos_owner (owner_user_id),INDEX idx_vendos_router (router_id),FOREIGN KEY(owner_user_id) REFERENCES users(id) ON DELETE SET NULL,FOREIGN KEY(router_id) REFERENCES routers(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
SQL);
        ...
        $this->db->exec("ALTER TABLE vendos ADD COLUMN IF NOT EXISTS client_subnet VARCHAR(64) NULL AFTER base_url");
