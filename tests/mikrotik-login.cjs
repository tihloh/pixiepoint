...
const fixtures=JSON.parse(execFileSync('php',[path.join(__dirname,'mikrotik-fixtures.php')],{encoding:'utf8'}));
const cases=fixtures.logins;
const md5=fs.readFileSync(path.join(root,'public/assets/md5.js'),'utf8');
for(const test of cases){
  let submits=0;
  const fields={username:{},password:{}};
  const form={elements:{namedItem:name=>fields[name]},submit(){submits++;}};
  const inputs={'compat-voucher':{value:'AbC123'},'pp-mikrotik-username':{value:'member'},'pp-mikrotik-password':{value:'secret'}};
  const sandbox={document:{forms:{namedItem:()=>form},getElementById:id=>inputs[id]||null,querySelectorAll:()=>[]},location:{search:''},URLSearchParams};
...
const sandbox={};vm.createContext(sandbox);vm.runInContext(encode,sandbox);
const allBytes=String.fromCharCode(...Array.from({length:256},(_,i)=>i));
const transported=new URLSearchParams(new URLSearchParams({chap:sandbox.chapBytes(allBytes)}).toString()).get('chap');
...
for(const test of fixtures.sessions){
 const sandbox={document:{querySelectorAll:()=>[],getElementById:()=>null}};sandbox.window=sandbox;vm.createContext(sandbox);
