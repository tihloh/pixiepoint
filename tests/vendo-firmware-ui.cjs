...
async function check(status) {
    ...
    const controls = {
      '[data-firmware-update]': update,
      '[data-firmware-latest]': latest,
      '[data-firmware-state]': state,
      '[data-firmware-error]': error,
      '[data-firmware-target]': target,
      '[data-firmware-checked]': checked
    };
    let onSubmit, requests = 0;
    const form = {
      querySelector(selector) {
        return selector === '[name=device_id]' ? {
          value: 'DEV-test'
        } : null;
      },
    ...
    const sandbox = {
      ...
      location: {
        pathname: '/admin/vendo-gateway',
        search: '?device=DEV-test'
      },
      CSS: {
        escape: value => value
      },
  ...
  (async () => {
    await check({
      ...
      target: 'esp32',
      checked_at: '2026-09-17T03:00:00Z'
    });
