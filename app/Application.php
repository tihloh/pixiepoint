...
final class Application
{
    public static function run(string $root): never
    {
        ...
        $routes->middleware('prefab.access', static function(callable $next,RouteMatch $match) use($auth,$view,$logs){$meta=$match->route()->metadata();if($meta['auth']??false)$auth->requireAccount();if(!empty($meta['permission']))$auth->requirePermission((string)$meta['permission'],$view);if(!empty($meta['log']))$logs->record(['action'=>(string)$meta['log'],'subject_type'=>'route','subject_id'=>$match->route()->routeName(),'actor_id'=>$auth->auth()->id(),'metadata'=>['path'=>$match->route()->path(),'method'=>$_SERVER['REQUEST_METHOD']??'GET'],'ip_address'=>$_SERVER['REMOTE_ADDR']??null,'user_agent'=>$_SERVER['HTTP_USER_AGENT']??null]);return $next();});
        (require $root.'/app/Routes/web.php')($routes,$controllers); (require $root.'/app/Routes/api.php')($routes,$controllers);
