...
final class Application
{
    public static function run(string $root): never
    {
        ...
        $logs = $prefab['logs']; $routes = $prefab['routes'];
        $vendoApi = new VendoApi($app->db); $routerQueue = new RouterCommandQueue($app->db);
        ...
        $controllers = [
            ...
            'admin.users' => new UsersController($app->db, $auth, $view, $logs, $prefab['users']),
            'admin.permissions' => new PermissionsController($app->db, $auth, $view, $logs, $prefab['permissions'], $prefab['users'], $root),
    ...
    {
        session_save_path(sys_get_temp_dir()); session_name($config['session_name'] ?? 'pixiepoint_session');
