...
final class PortalFeatureConfig
{
    ...
    public function resolve(int $routerId,?int $stationId=null): array{$router=$this->read('router',$routerId);$station=$stationId?$this->read('station',$stationId):[];$out=[];foreach(self::KEYS as $key){$base=$router[$key]??['value'=>false,'config'=>[]];$over=$station[$key]??null;$custom=$over!==null&&$over['value']!==null;$out[$key]=['enabled'=>(bool)($custom?$over['value']:$base['value']),'source'=>$custom?'station':'router','config'=>array_replace($base['config'],$custom?($over['config']??[]):[])];}if(!$out['points']['enabled'])foreach(['points_convert','points_play','points_share'] as $key)$out[$key]['enabled']=false;return $out;}
