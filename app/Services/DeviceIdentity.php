...
final class DeviceIdentity
{
    ...
    public function claimAsNew(int $deviceId, int $userId): void
    {
        ...
        if ($device['user_id'] !== null && (int) $device['user_id'] !== $userId) {
            throw new RuntimeException('This device is already linked to another account.');
        }
    ...
    public function mergeInto(int $sourceId, int $targetId, int $userId): void
    {
        ...
        if ((int) ($target['user_id'] ?? 0) !== $userId) {
            throw new RuntimeException('The target device does not belong to your account.');
        }
        if ($source['user_id'] !== null && (int) $source['user_id'] !== $userId) {
            throw new RuntimeException('The detected device belongs to another account.');
        }
    ...
    private function deviceByMac(string $mac, string $scopeKey): ?array
    {
        ...
        $stmt->execute([$mac]);
        $device = $stmt->fetch();
        if ($device && (int) $device['matches'] === 1) {
            return $device;
        }
