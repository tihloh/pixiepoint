...
final class BusinessName
{
    ...
    {
        $stmt = $this->db->prepare('SELECT business_name,router_id FROM vendos WHERE id=? LIMIT 1');
        $stmt->execute([$vendoId]);
        $vendo = $stmt->fetch();
        if (!$vendo) return '';
        $name = trim((string) ($vendo['business_name'] ?? ''));
        return $name !== '' ? $name : $this->router((int) $vendo['router_id']);
    }

    private function ensureSchema(): void
    {
        if (self::$schemaReady) return;
        $this->db->exec('ALTER TABLE vendos ADD COLUMN IF NOT EXISTS business_name VARCHAR(255) NULL AFTER name');
