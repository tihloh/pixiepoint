...
<body>
    <?php if ($dashboard): ?>
        <nav class="navbar border-bottom d-lg-none sticky-top">
            <div class="container-fluid px-3"><a class="navbar-brand fw-bold d-flex align-items-center gap-2"
                    href="/dashboard"><span class="logo">P</span><span><?= $name ?></span></a><button class="navbar-toggler"
                    type="button" data-bs-toggle="offcanvas" data-bs-target="#pixiepoint-sidebar"
                    aria-controls="pixiepoint-sidebar" aria-label="Open navigation"><span
                        class="navbar-toggler-icon"></span></button></div>
        </nav>
        <div class="d-lg-flex min-vh-100">
            <aside class="offcanvas-lg offcanvas-start border-end pixie-sidebar" tabindex="-1" id="pixiepoint-sidebar"
                aria-labelledby="pixiepoint-sidebar-label">
                <div class="offcanvas-header border-bottom d-lg-none">
                    <h5 class="offcanvas-title" id="pixiepoint-sidebar-label"><?= $name ?></h5><button type="button"
                        class="btn-close" data-bs-dismiss="offcanvas" data-bs-target="#pixiepoint-sidebar"
                        aria-label="Close"></button>
                </div>
                <div class="offcanvas-body d-flex flex-column p-3">
                    <a class="d-none d-lg-flex flex-column align-items-center text-decoration-none text-body fw-bold mb-4 px-2 pixie-brand"
                        href="/dashboard"><span class="logo pixie-brand-logo">P</span><span
                            class="mt-2"><?= $name ?></span></a>
                    <nav class="nav nav-pills flex-column gap-1">
                        ...
                        <?php if ($isOverview): ?>
                            <?php if ($access['routers'] ?? false): ?><div class="nav-group-label mt-3">Network</div><a
                                    class="nav-link<?= $active('/admin/routers') ?>"
                                    href="/admin/routers">Routers</a><?php endif; ?>
                        <?php elseif ($access['routers'] ?? false): ?>
                            ...
                            <form method="get" action="/admin/select/router" class="px-2 pt-1 pb-2" id="sidebar-router-form">
                                <input type="hidden" name="return" value="<?= e($sidebarReturn) ?>"><label
                                    class="visually-hidden" for="sidebar-router-select">Select router</label><select
                                    class="form-select form-select-sm" id="sidebar-router-select" name="select">
                                    <option value="">Select router…</option><?php foreach ($sidebarRouters as $router): ?>
                                        <option value="<?= e($router['id']) ?>"
                                            <?= $selectedRouter && (int)$selectedRouter['id'] === (int)$router['id'] ? 'selected' : '' ?>>
                                            <?= e($router['name']) ?> · <?= e($router['identity']) ?></option><?php endforeach; ?>
                                    <option value="__add__">＋ Add new router</option>
                                </select>
                            </form>
                            <?php if ($selectedRouter): ?>
                                <div class="small text-body-secondary mt-1 px-2 text-truncate"
                                    title="<?= e($selectedRouter['identity']) ?>"><?= e($selectedRouter['identity']) ?></div>
                                ...
                                <?php if ($access['devices'] ?? false): ?><a class="nav-link<?= $active('/admin/devices') ?>"
                                        href="/admin/devices">Devices</a><?php endif; ?>
                                <?php if ($access['sessions'] ?? false): ?><a class="nav-link<?= $active('/admin/sessions') ?>"
                                        href="/admin/sessions">Sessions</a><?php endif; ?>
                            <?php endif; ?>
                        <?php endif; ?>
                        <?php if (($access['users'] ?? false) || ($access['groups'] ?? false)): ?><div
                                class="nav-group-label mt-3">Administration</div><?php if ($access['users'] ?? false): ?><a
                                    class="nav-link<?= $active('/admin/users') ?>"
                                    href="/admin/users">Users</a><?php endif; ?><?php endif; ?>
                                <?php if ($access['logs'] ?? false): ?><div class="nav-group-label mt-3">System</div><a
                                        class="nav-link<?= $active('/admin/logs') ?>" href="/admin/logs">Logs</a><?php endif; ?>
                    </nav>
                    <div class="card border-0 rounded-3 mt-auto pixie-user-card">
                        <div class="card-body p-3">
                            <div class="d-flex align-items-center gap-3"><?php if ($sidebarAvatar !== ''): ?><img
                                        src="<?= e($sidebarAvatar) ?>" alt="" class="pixie-user-avatar"><?php else: ?><span
                                        class="pixie-user-avatar pixie-user-avatar-fallback"
                                        aria-hidden="true"><?= e($sidebarInitial) ?></span><?php endif; ?><div
                                    ...
                                    <?php if ($sidebarPoints !== null): ?><div class="small mt-1"><span
                                                aria-hidden="true">★</span> <?= e(number_format($sidebarPoints)) ?> points</div>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="d-flex gap-2 mt-3"><a
                                    class="btn btn-outline-light btn-sm flex-fill<?= $active('/profile') ?>"
                                    href="/profile">Profile</a><a class="btn btn-outline-danger btn-sm flex-fill"
