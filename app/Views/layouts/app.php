...
<head>
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet" crossorigin="anonymous"><link rel="stylesheet" href="/assets/app.css?v=<?= e($cssVersion) ?>"><link rel="stylesheet" href="/assets/admin.css?v=<?= e($cssVersion) ?>">
    <script src="https://cdn.jsdelivr.net/npm/jquery@3.7.1/dist/jquery.min.js" crossorigin="anonymous"></script><script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js" crossorigin="anonymous"></script>
</head>
<body>
<?php if ($dashboard): ?>
        ...
        <aside class="offcanvas-lg offcanvas-start border-end pixie-sidebar" tabindex="-1" id="pixiepoint-sidebar" aria-labelledby="pixiepoint-sidebar-label">
            ...
            <div class="offcanvas-body d-flex flex-column p-3">
                ...
                <nav class="nav nav-pills flex-column gap-1">
                    ...
                    <?php if ($access['routers'] ?? false): ?><a class="nav-link<?= $active('/admin/portal-themes') ?>" href="/admin/portal-themes">Portal Themes</a><?php endif; ?>

                    <?php if ($isOverview): ?>
                        <?php if ($access['routers'] ?? false): ?><div class="nav-group-label mt-3">Network</div><a class="nav-link<?= $active('/admin/routers') ?>" href="/admin/routers">Routers</a><?php endif; ?>
                    <?php elseif ($access['routers'] ?? false): ?>
                        <div class="nav-group-label mt-3">Network</div>
                        <form method="get" action="/admin/select/router" class="px-2 pt-1 pb-2" id="sidebar-router-form"><input type="hidden" name="return" value="<?= e($sidebarReturn) ?>"><label class="visually-hidden" for="sidebar-router-select">Select router</label><select class="form-select form-select-sm" id="sidebar-router-select" name="select"><option value="">Select router…</option><?php foreach ($sidebarRouters as $router): ?><option value="<?= e($router['id']) ?>" <?= $selectedRouter && (int)$selectedRouter['id']===(int)$router['id']?'selected':'' ?>><?= e($router['name']) ?> · <?= e($router['identity']) ?></option><?php endforeach; ?><option value="__add__">＋ Add new router</option></select></form>
                        ...
                        <?php endif; ?>
                    <?php endif; ?>
                    <?php if (($access['users'] ?? false) || ($access['groups'] ?? false)): ?><div class="nav-group-label mt-3">Administration</div><?php if ($access['users'] ?? false): ?><a class="nav-link<?= $active('/admin/users') ?>" href="/admin/users">Users</a><?php endif; ?><?php if ($access['groups'] ?? false): ?><a class="nav-link<?= $active('/admin/groups') ?>" href="/admin/groups">Groups</a><?php endif; ?><?php endif; ?>
                    <?php if ($access['logs'] ?? false): ?><div class="nav-group-label mt-3">System</div><a class="nav-link<?= $active('/admin/logs') ?>" href="/admin/logs">Logs</a><?php endif; ?>
                </nav>
                <div class="card border-0 rounded-3 mt-auto pixie-user-card"><div class="card-body p-3"><div class="d-flex align-items-center gap-3"><?php if ($sidebarAvatar !== ''): ?><img src="<?= e($sidebarAvatar) ?>" alt="" class="pixie-user-avatar"><?php else: ?><span class="pixie-user-avatar pixie-user-avatar-fallback" aria-hidden="true"><?= e($sidebarInitial) ?></span><?php endif; ?><div class="min-w-0 flex-grow-1"><div class="fw-semibold text-truncate"><?= e($sidebarName) ?></div><div class="text-body-secondary small text-truncate"><?= e($sidebarRole) ?></div><?php if ($sidebarPoints !== null): ?><div class="small mt-1"><span aria-hidden="true">★</span> <?= e(number_format($sidebarPoints)) ?> points</div><?php endif; ?></div></div><div class="d-flex gap-2 mt-3"><a class="btn btn-outline-light btn-sm flex-fill<?= $active('/profile') ?>" href="/profile">Profile</a><a class="btn btn-outline-danger btn-sm flex-fill" href="/logout">Log out</a></div></div></div>
            </div>
        </aside>
...
<script>
$(function(){ $('#sidebar-router-select').on('change',function(){ if(this.value==='__add__'){const modal=document.getElementById('register-router-modal');if(modal&&window.bootstrap?.Modal){window.bootstrap.Modal.getOrCreateInstance(modal).show();return;}window.location.href='/dashboard?register=1';}else{$('#sidebar-router-form').trigger('submit');}}); });
</script>
</body>
</html>
