...
return static function (RouteManager $routes, array $c): void {
    $nativeUrl = static function (mixed $value): ?string {
        $url = trim((string)$value); $parts = $url !== '' ? parse_url($url) : false;
        if (!$parts) return null;
        $scheme = strtolower((string)($parts['scheme'] ?? '')); $host = trim((string)($parts['host'] ?? ''));
    ...
    $routes->post('/hotspot/session', static function () use ($c, $nativeUrl): never { $refreshUrl=$nativeUrl($_POST['refresh_url']??null); if($refreshUrl!==null){header('Location: '.$refreshUrl,true,303);exit;} $c['hotspot']->session(); })->name('hotspot.session');
    $routes->post('/hotspot/disconnected', static function () use ($c, $nativeUrl): never { $loginUrl=$nativeUrl($_POST['login_url']??null); if($loginUrl!==null){header('Location: '.$loginUrl,true,303);exit;} $c['hotspot']->disconnected(); })->name('hotspot.disconnected');
    ...
    $routes->redirect('/admin/login','/'); $routes->redirect('/admin/logout','/logout'); $routes->redirect('/admin','/dashboard');
    ...
    foreach(['Routers','Vendos','Vouchers','Devices','Sessions','Sales','Logs'] as $feature) (require $adminRoot.'/'.$feature.'/routes.php')($routes,$c);
};
