...
return static function (RouteManager $routes, array $c): void {
    // Only allow native HTTP(S) URLs before redirecting a hotspot client back
    // to a RouterOS-provided URL.
    ...
    // -------------------------------------------------------------------------
    // Public portal and hotspot flow
    // -------------------------------------------------------------------------
    ...
    $routes
        ->post('/', [$c['hotspot'], 'portal'])
        ->name('hotspot.portal');

    $routes
        ->get('/hotspot/compat', [$c['vendos.hotspot'], 'portal'])
        ->name('hotspot.compat');

    $routes
        ->get('/hotspot/device-info', [$c['device_info'], 'show'])
        ->name('hotspot.device_info');

    $routes
        ->post('/hotspot/device-voucher', [$c['device_info'], 'saveVoucher'])
        ->name('hotspot.device_voucher');

    $routes
        ->post('/hotspot/authenticate', [$c['hotspot'], 'authenticate'])
        ->name('hotspot.authenticate');
    ...
    $routes
        ->matchMethods(['GET', 'POST'], '/setup', [$c['auth'], 'setup'])
        ->name('setup');

    $routes
        ->matchMethods(['GET', 'POST'], '/register', [$c['auth'], 'register'])
        ->name('register');
    ...
    $routes
        ->get('/login', static fn () => redirect('/'))
        ->name('login');

    $routes
        ->get('/logout', [$c['auth'], 'logout'])
        ->name('logout')
        ->auth()
        ->middleware('prefab.access');

    $routes
        ->get('/auth/google', [$c['auth'], 'googleStart'])
        ->name('auth.google');

    $routes
        ->get('/auth/google/callback', [$c['auth'], 'googleCallback'])
    ...
    $routes
        ->get('/dashboard', [$c['dashboard'], 'index'])
        ->name('dashboard')
        ->auth()
        ->middleware('prefab.access');

    $routes
        ->post('/devices/claim', [$c['dashboard'], 'claimDevice'])
        ->name('devices.claim')
        ->auth()
        ->middleware('prefab.access');
