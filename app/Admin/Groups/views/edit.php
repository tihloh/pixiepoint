...
<div class="row justify-content-center">
        ...
        <div class="card border shadow bg-body-tertiary overflow-hidden">
            ...
            <form method="post">
                <div class="card-body p-4">
                    <input type="hidden" name="_csrf" value="<?=e($csrf)?>"><?= $message ?>
                    <div class="mb-3">
                        <label class="form-label" for="group-name">Group Name</label>
                        <input class="form-control" id="group-name" name="name" value="<?=e($group->name)?>" required>
                    </div>
                    <div class="mb-4">
                        <label class="form-label" for="group-description">Description</label>
                        <input class="form-control" id="group-description" name="description" value="<?=e($group->description ?? '')?>">
                    </div>
    ...
    $value = array_key_exists($permission, $overrides) ? ($overrides[$permission] ? 'allow' : 'deny') : 'inherit';?><tr>
                                        <td>
                                            <strong><?=e($definition['name'] ?? $permission)?></strong><?php if (!empty($definition['description'])) :
    ?><div class="small text-body-secondary"><?=e($definition['description'])?></div><?php endif;?>
                                        </td>
                                        <td>
