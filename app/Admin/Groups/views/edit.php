...
<?php
foreach($definitions as $permission=>$definition):$value=array_key_exists($permission,$overrides)?($overrides[$permission]?'allow':'deny'):'inherit';?><tr><td><strong><?=e($definition['name']??$permission)?></strong><?php
if(!empty($definition['description'])):?><div class="small text-body-secondary"><?=e($definition['description'])?></div><?php
