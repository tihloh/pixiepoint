...
<div class="row justify-content-center">
    <div class="col-xl-7 col-lg-8">
        <div class="card border shadow bg-body-tertiary overflow-hidden">
            <div class="card-header d-flex justify-content-between align-items-center px-4 py-3">
                <h1 class="h4 mb-0">Edit Group</h1><a class="btn btn-sm btn-outline-secondary" href="/admin/groups">Back</a>
            </div>
            <form method="post">
                <div class="card-body p-4"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><?= $message ?>
    ...
    $value = array_key_exists($permission, $overrides) ? ($overrides[$permission] ? 'allow' : 'deny') : 'inherit';?><tr>
                                        <td><strong><?=e($definition['name'] ?? $permission)?></strong><?php if (!empty($definition['description'])) :
    ?><div class="small text-body-secondary"><?=e($definition['description'])?></div><?php endif;?></td>
                                        <td><select class="form-select form-select-sm" name="permissions[<?=e($permission)?>]">
                ...
                <div class="card-footer d-flex justify-content-end gap-2 px-4 py-3"><a class="btn btn-outline-secondary" href="/admin/groups">Cancel</a><button class="button" type="submit">Save Changes</button></div>
            </form>
        </div>
    </div>
</div>