...
abstract class FeatureController
{
    ...
    private function logBelongsToRouter(array $log,int $routerId):bool
    {
        ...
        $subjectType=(string)($log['subject_type']??'');$subjectId=max(0,(int)($log['subject_id']??0));
        if($subjectId<1)return false;
        $lookup=match($subjectType){
            'voucher','voucher_batch','session','router_login_event'=>'SELECT router_id FROM '.($subjectType==='voucher_batch'?'voucher_batches':($subjectType==='router_login_event'?'router_login_events':$subjectType.'s')).' WHERE id=? LIMIT 1',
            'station','vendo'=>'SELECT router_id FROM vendos WHERE id=? LIMIT 1',
            default=>null,
        };
        if($lookup===null)return false;$stmt=$this->db->prepare($lookup);$stmt->execute([$subjectId]);return(int)$stmt->fetchColumn()===$routerId;
    }
