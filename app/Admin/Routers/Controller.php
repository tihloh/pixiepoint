...
final class Controller extends FeatureController
{
    ...
    public function index(): never
    {
        ...
        if($selectId>0){
            if(!$access->canView($selectId,$userId,$platformOwner)){
                $_SESSION['admin_flash']='<div class="alert">Gateway not found or access denied.</div>';
                redirect('/admin/routers');
            }
            $stmt=$this->db->prepare('SELECT id FROM routers WHERE id=? AND enabled=1 LIMIT 1');
            $stmt->execute([$selectId]);
            if(!$stmt->fetchColumn()){
                $_SESSION['admin_flash']='<div class="alert">Gateway is unavailable.</div>';
                redirect('/admin/routers');
            }
        ...
        if($this->isPost()){
            ...
            if($action!=='update'){
                $_SESSION['admin_flash']='<div class="alert">Register new MikroTik gateways from the RouterOS Terminal command on your dashboard.</div>';
                redirect('/admin/routers');
            }
            ...
            if($result->fails())$message=$this->errors($result->errors());
                ...
                try{
                    ...
                    if($id<1||!$access->canManage($id,$userId,$platformOwner))throw new RuntimeException('Gateway not found or access denied.');
                    ...
                    $this->audit('router.updated','router',$id,'MikroTik gateway was updated.');
                    $message='<div class="alert ok">Gateway updated.</div>';
                }
                catch(Throwable $e){
                    $message='<div class="alert">The gateway could not be saved. '.e($e->getMessage()).'</div>';
                }
            }
    ...
    public function settings(int|string $id): never
    {
        ...
        try{
            if($action==='portal_features'){
                ...
                $this->audit('router.portal_features.updated','router',$routerId,'Gateway portal feature defaults were updated.');
                ...
                $this->audit('router.updated','router',$routerId,'MikroTik gateway settings were updated.');
                $_SESSION['admin_flash']='<div class="alert ok">Gateway settings updated.</div>';
            }
        }
        catch(Throwable $e){
    ...
    public function dashboard(int|string $id): never
    {
        ...
        $gatewayVendos=$bridge->bindings($routerId);
        $metrics=[];
        ...
        foreach($metricQueries as $key=>[$label,$permission,$sql]){
            if(!$this->auth->can($permission.'.view'))continue;
            $stmt=$this->db->prepare($sql);
            $stmt->execute(substr_count($sql,'?')===2?[$routerId,$routerId]:[$routerId]);
            $metrics[$key]=['label'=>$label,'value'=>$stmt->fetchColumn()];
        }
        if($this->auth->can('vendos.view'))$metrics['vendos']=['label'=>'Vendos','value'=>count($gatewayVendos)];
        $recentSessions=[];
        ...
        $features=new PortalFeatureConfig($this->db);
        $stmt=$this->db->prepare('SELECT v.*,r.identity router_identity,x.gateway_device_id vendo_device_id,x.name vendo_name FROM vendos v JOIN routers r ON r.id=v.router_id LEFT JOIN vendo_gateway_vendos x ON x.station_id=v.id WHERE v.router_id=? ORDER BY v.created_at DESC');
        $stmt->execute([$routerId]);
