...
<form method="post">
    ...
    <section class="panel mb-4">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6 field"><label for="router-theme">Portal theme</label><select id="router-theme" name="portal_theme_id"><option value="0">System default</option><?php
            foreach ($themes as $theme) :
                ?><option value="<?= e($theme['id']) ?>" <?= (int)($router['portal_theme_id'] ?? 0) === (int)$theme['id'] ? 'selected' : '' ?>><?= e($theme['name']) ?></option><?php
            endforeach; ?></select><small class="text-body-secondary mt-1 d-block">Stations can override this individually.</small></div>
        </div>
    ...
    <section class="panel">
        ...
        <div class="row gx-4 gy-3">
            ...
            foreach ($featureLabels as $key => $label) :
                ?><div class="col-md-4"><div class="form-check form-switch"><input class="form-check-input flex-shrink-0" style="width:2.5em;height:1.25em" type="checkbox" name="<?= e($key) ?>" value="1" id="router-<?= e($key) ?>" <?= $featureOn($key) ? 'checked' : '' ?>><label class="form-check-label ms-2" for="router-<?= e($key) ?>"><?= e($label) ?></label></div></div><?php
            endforeach; ?>
