...
<form method="post">
    ...
    <section class="panel mb-4">
        ...
        <div class="form-grid">
            <div class="field"><label for="router-name">Router / Wi-Fi name</label><input id="router-name" name="name" value="<?= e($router['name']) ?>" required maxlength="160"><small class="text-body-secondary">Used as the router's display name and top-level Wi-Fi/business identity.</small></div>
            <div class="field"><label for="router-identity">RouterOS identity</label><input id="router-identity" value="<?= e($router['identity']) ?>" readonly></div>
            <div class="field"><label for="router-host">Public hostname / VPN IP</label><input id="router-host" name="public_host" value="<?= e($router['public_host'] ?? '') ?>" placeholder="router.example.com or 10.10.0.2" maxlength="255"></div>
