...
document.getElementById('routerModal').addEventListener('show.bs.modal', function (event) {
    ...
    form.reset();
    form.action.value = edit ? 'update' : 'create';
    form.id.value = edit ? button.dataset.id : '0';
    form.name.value = edit ? button.dataset.name : '';
    form.identity.value = edit ? button.dataset.identity : '';
    form.public_host.value = edit ? button.dataset.host : '';
    form.location.value = edit ? button.dataset.location : '';
    form.enabled.checked = edit ? button.dataset.enabled === '1' : true;
