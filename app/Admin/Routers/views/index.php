...
<section class="panel">
    ...
    <div class="table-responsive">
        <table class="table align-middle">
            ...
            <tbody>
                <?php if (!$routers): ?><tr>
                    <td colspan="5" class="empty">No gateways linked to this account yet.</td>
                </tr><?php endif; ?>
                ...
                <tr role="button" tabindex="0" onclick="window.location.href='/admin/routers/<?= e($router['id']) ?>'" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.location.href='/admin/routers/<?= e($router['id']) ?>'}">
                    <td>
                        <a class="text-decoration-none text-body" href="/admin/routers/<?= e($router['id']) ?>">
                            <strong><?= e($router['name']) ?></strong>
                            <div class="small text-body-secondary"><?= e($router['location'] ?: 'No location set') ?></div>
                        </a>
                    </td>
                    <td class="code"><?= e($router['identity']) ?></td>
                    <td><?= e($router['public_host'] ?: 'Not set') ?></td>
                    <td>
                        <span class="badge <?= $router['enabled'] ? '' : 'off' ?>"><?= $router['enabled'] ? 'Enabled' : 'Disabled' ?></span>
                    </td>
