<?php
/** @var array $vendos */ /** @var array $routers */ /** @var array $themes */ /** @var bool $canManageVendos */ /** @var string $message */ /** @var string $csrf */
...
?>
<style>
    .station-switches .form-check-input,
    .debug-switch .form-check-input {
        width: 2em;
        min-width: 2em;
        height: 1em;
        margin-top: .25em;
...
<div class="heading">
    <div>
        ...
        <p class="muted">Configure each hotspot station, optional Vendo hardware, and station-level portal feature overrides.</p>
    </div><?php if($canManageVendos):?><button class="button" type="button" data-bs-toggle="modal" data-bs-target="#stationModal" data-mode="create">Add station</button><?php endif;?>
</div>
<?= $message ?>
    ...
    <div class="table-responsive">
        <table class="table align-middle">
            <thead>
                <tr>
                    <th>Name</th>
                    <th>Vendo hardware</th>
                    <th>Server</th>
                    <th>Theme</th>
                    <th>Features</th>
                    <th>Status</th><?php if($canManageVendos):?><th>Debug portal</th>
                    <th></th><?php endif;?>
                </tr>
            </thead>
            <tbody>
...
$shown=0;
foreach($featureLabels as $key=>$label):$on=(bool)($resolved[$key]['enabled']??false);
if($key==='coin_slot'&&!$hasVendo)$on=false;
if(!$on)continue;
$shown++;?><span class="badge text-bg-success"><?= e($label) ?></span><?php endforeach;?><?php if($shown===0):?><span class="text-body-secondary small">None</span><?php endif;?></div>
                ...
                <?php if(!$vendos):?><tr>
                    <td colspan="<?= $canManageVendos?8:6 ?>" class="empty">No hotspot stations configured.</td>
                </tr><?php endif;?></tbody>
        </table>
    </div>
</section>
...
<script>
    ...
    fm.addEventListener('show.bs.modal', e => {
        const b = e.relatedTarget,
            ...
            data = JSON.parse(b.dataset.features || '{}');
        ...
        f.querySelectorAll('[data-feature]').forEach(s => {
            const v = data[s.dataset.feature]?.value;
            s.value = v === true ? 'on' : v === false ? 'off' : 'inherit';
        });
        f.querySelector('[name=trial_minutes]').value = data.trial?.config?.minutes || 10;
        f.querySelector('[name=convert_points]').value = data.points_convert?.config?.points || 10;
        f.querySelector('[name=convert_minutes]').value = data.points_convert?.config?.minutes || 5;
    });
    ...
    sm.addEventListener('show.bs.modal', e => {
        ...
        if (edit) {
            ...
            f.querySelector('[name=router_id]').value = b.dataset.router;
            ...
            f.querySelector('[name=base_url]').value = b.dataset.url;
            f.querySelector('[name=charging_enabled]').checked = b.dataset.charging === '1';
            f.querySelector('[name=eload_enabled]').checked = b.dataset.eload === '1';
