...
final class Controller extends FeatureController
{
    ...
    private function saveStationFeatures(PortalFeatureConfig $features,RouterAccess $access,int $userId,bool $platformOwner,int $routerId): string
    {
        ...
        if(!$station||(int)$station['router_id']!==$routerId||!$access->canManage($routerId,$userId,$platformOwner))return '<div class="alert">Station not found or access denied.</div>';
        $features->saveStation($id,$_POST);
        $this->audit('station.portal_features.updated','station',$id,'Station access methods were updated.');
        return '<div class="alert ok">Station access methods updated.</div>';
    }
    private function toggleDebug(RouterAccess $access,int $userId,bool $platformOwner,int $routerId): string
    {
        ...
        $station=$this->findStation($id);
        if(!$station||(int)$station['router_id']!==$routerId||!$access->canManage($routerId,$userId,$platformOwner))return '<div class="alert">Station not found or access denied.</div>';
        $this->db->prepare('UPDATE vendos SET debug_enabled=? WHERE id=?')->execute([$enabled?1:0,$id]);
    ...
    private function saveStation(RouterAccess $access,int $userId,bool $platformOwner,string $action,int $selectedRouterId): string
    {
        ...
        if($routerId!==$selectedRouterId)return '<div class="alert">Select the router before managing its stations.</div>';
        ...
        if($subnet!==''&&!$this->validCidr($subnet))return '<div class="alert">Client subnet must be valid CIDR.</div>';
        ...
        try{
            $check=$this->db->prepare('SELECT id FROM routers WHERE id=?');
            $check->execute([$routerId]);
            if(!$check->fetchColumn())throw new RuntimeException('Router unavailable.');
            ...
            if($action==='update'){
                ...
                if(!$existing||(int)$existing['router_id']!==$selectedRouterId)throw new RuntimeException('Station not found.');
                $stmt=$this->db->prepare('UPDATE vendos SET router_id=?,name=?,server_ip=?,client_subnet=?,interface_name=?,portal_theme_id=?,password_mode=?,enabled=? WHERE id=?');
                $stmt->execute([...$params,$id]);
                return '<div class="alert ok">Station updated.</div>';
            }
            $stmt=$this->db->prepare("INSERT INTO vendos(owner_user_id,router_id,name,base_url,server_ip,client_subnet,interface_name,portal_theme_id,password_mode,charging_enabled,eload_enabled,enabled) VALUES(NULL,?,?,'',?,?,?,?,?,0,0,?)");
            $stmt->execute($params);
