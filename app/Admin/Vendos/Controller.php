...
final class Controller extends FeatureController
{
    ...
    private function saveStation(RouterAccess $access,int $userId,bool $platformOwner,string $action,int $selectedRouterId): string
    {
        ...
        if($routerId!==$selectedRouterId)return '<div class="alert">Select the router before managing its stations.</div>';if(!in_array($type,['hotspot','vendo_gateway'],true))return '<div class="alert">Invalid station type.</div>';if(filter_var($serverIp,FILTER_VALIDATE_IP)===false)return '<div class="alert">Server IP must be a valid IPv4 or IPv6 address.</div>';if($subnet!==''&&!$this->validCidr($subnet))return '<div class="alert">Client subnet must be valid CIDR.</div>';if(!in_array((string)$data['password_mode'],['blank','voucher'],true))return '<div class="alert">Invalid password mode.</div>';if(!$access->canManage($routerId,$userId,$platformOwner))return '<div class="alert">You do not have access to manage that router.</div>';
        $baseUrl='';if($type==='vendo_gateway'&&trim((string)($data['base_url']??''))!==''){$baseUrl=$this->normalizeBaseUrl((string)$data['base_url'])??'';if($baseUrl==='')return '<div class="alert">Controller address is invalid.</div>';}$charging=$type==='vendo_gateway'?(int)($data['charging_enabled']??0):0;$eload=$type==='vendo_gateway'?(int)($data['eload_enabled']??0):0;
        try{$check=$this->db->prepare('SELECT id FROM routers WHERE id=?');$check->execute([$routerId]);if(!$check->fetchColumn())throw new RuntimeException('Router unavailable.');$params=[$routerId,$data['name'],$type,$baseUrl,$serverIp,$subnet?:null,$data['interface_name']??null,$themeId?:null,$data['password_mode'],$charging,$eload,(int)($data['enabled']??0)];
            if($action==='update'){$id=max(0,(int)($_POST['id']??0));$existing=$this->findStation($id);if(!$existing||(int)$existing['router_id']!==$selectedRouterId)throw new RuntimeException('Station not found.');$stmt=$this->db->prepare('UPDATE vendos SET router_id=?,name=?,station_type=?,base_url=?,server_ip=?,client_subnet=?,interface_name=?,portal_theme_id=?,password_mode=?,charging_enabled=?,eload_enabled=?,enabled=? WHERE id=?');$stmt->execute([...$params,$id]);return '<div class="alert ok">Station updated.</div>';}
            $stmt=$this->db->prepare('INSERT INTO vendos(owner_user_id,router_id,name,station_type,base_url,server_ip,client_subnet,interface_name,portal_theme_id,password_mode,charging_enabled,eload_enabled,enabled) VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?)');$stmt->execute($params);return '<div class="alert ok">Station added.</div>';
        }catch(Throwable $e){return '<div class="alert">The station could not be saved. '.e($e->getMessage()).'</div>';}
    }

    private function findStation(int $id): ?array{if($id<1)return null;$stmt=$this->db->prepare('SELECT id,router_id,name,station_type FROM vendos WHERE id=? LIMIT 1');$stmt->execute([$id]);$row=$stmt->fetch();return $row?:null;}
