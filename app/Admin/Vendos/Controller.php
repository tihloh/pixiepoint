...
final class Controller extends FeatureController
{
    ...
    public function index(): never
    {
        ...
        $_SESSION['admin_flash']=$message;redirect('/admin/routers/'.$routerId.'#stations');
    }
    ...
    {
        $id=max(0,(int)($_POST['id']??0));$station=$this->findStation($id);if(!$station||(int)$station['router_id']!==$routerId||!$access->canManage($routerId,$userId,$platformOwner))return '<div class="alert">Station not found or access denied.</div>';
        $features->saveStation($id,$_POST);$this->audit('station.portal_features.updated','station',$id,'Station access methods were updated.');return '<div class="alert ok">Station access methods updated.</div>';
    }

    private function toggleDebug(RouterAccess $access,int $userId,bool $platformOwner,int $routerId): string
    {
        $id=max(0,(int)($_POST['id']??0));$enabled=(string)($_POST['debug_enabled']??'0')==='1';$station=$this->findStation($id);if(!$station||(int)$station['router_id']!==$routerId||!$access->canManage($routerId,$userId,$platformOwner))return '<div class="alert">Station not found or access denied.</div>';
        $this->db->prepare('UPDATE vendos SET debug_enabled=? WHERE id=?')->execute([$enabled?1:0,$id]);return '<div class="alert ok">Debug '.($enabled?'enabled':'disabled').' for '.e($station['name']).'.</div>';
    }

    private function saveStation(RouterAccess $access,int $userId,bool $platformOwner,string $action,int $selectedRouterId): string
    {
        $result=Input::fromRequest()->process(['name'=>'trim|required|string|max:160','portal_theme_id'=>'default:0|integer|min:0','router_id'=>'required|integer|min:1','server_ip'=>'trim|required|string|max:45','client_subnet'=>'trim|null_if_empty|nullable|string|max:64','interface_name'=>'trim|null_if_empty|nullable|string|max:128','password_mode'=>'trim|required|string|max:32','enabled'=>'default:0|integer|min:0|max:1']);if($result->fails())return $this->errors($result->errors());
        ...
        try{
            $check=$this->db->prepare('SELECT id FROM routers WHERE id=?');$check->execute([$routerId]);if(!$check->fetchColumn())throw new RuntimeException('Router unavailable.');
            $params=[$routerId,$data['name'],$serverIp,$subnet?:null,$data['interface_name']??null,$themeId?:null,$data['password_mode'],(int)($data['enabled']??0)];
            ...
            $stmt=$this->db->prepare("INSERT INTO vendos(owner_user_id,router_id,name,base_url,server_ip,client_subnet,interface_name,portal_theme_id,password_mode,charging_enabled,eload_enabled,enabled) VALUES(NULL,?,?, '',?,?,?,?,?,?,0,0,?)");$stmt->execute($params);return '<div class="alert ok">Station added.</div>';
