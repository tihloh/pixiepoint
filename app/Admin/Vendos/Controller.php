...
final class Controller extends FeatureController
{
    ...
    private function saveStation(RouterAccess $access,int $userId,bool $platformOwner,string $action,int $selectedRouterId): string
    {
        ...
        $baseUrl='';if(trim((string)($data['base_url']??''))!==''){$baseUrl=$this->normalizeBaseUrl((string)$data['base_url'])??'';if($baseUrl==='')return '<div class="alert">Controller address is invalid.</div>';}$charging=$type==='vendo_gateway'?(int)($data['charging_enabled']??0):0;$eload=$type==='vendo_gateway'?(int)($data['eload_enabled']??0):0;
