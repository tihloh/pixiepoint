  ...
  if($this->isPost()){
   ...
   if($action==='toggle_debug'){
    ...
    if($result->fails())$message=$this->errors($result->errors());else{
     $data=$result->validated();$serverIp=trim((string)$data['server_ip']);$subnet=trim((string)($data['client_subnet']??''));$baseUrl=$this->normalizeBaseUrl((string)$data['base_url']);
     ...
     elseif(!in_array((string)$data['password_mode'],['blank','voucher'],true))$message='<div class="alert">Invalid password mode.</div>';
      ...
      if($action==='update'){
       $id=max(0,(int)($_POST['id']??0));if($id<1)throw new RuntimeException('Vendo not found.');$sql='UPDATE vendos SET router_id=?,name=?,base_url=?,server_ip=?,client_subnet=?,interface_name=?,password_mode=?,charging_enabled=?,eload_enabled=?,enabled=? WHERE id=?';$params=[(int)$data['router_id'],$data['name'],$baseUrl,$serverIp,$subnet?:null,$data['interface_name']??null,$data['password_mode'],(int)($data['charging_enabled']??0),(int)($data['eload_enabled']??0),(int)($data['enabled']??0),$id];if(!$platformOwner){$sql.=' AND owner_user_id=?';$params[]=$userId;}$stmt=$this->db->prepare($sql);$stmt->execute($params);if($stmt->rowCount()<1){$exists=$this->db->prepare('SELECT id FROM vendos WHERE id=?'.($platformOwner?'':' AND owner_user_id=?'));$exists->execute($platformOwner?[$id]:[$id,$userId]);if(!$exists->fetchColumn())throw new RuntimeException('Vendo not found.');}$this->audit('vendo.updated','vendo',$id,'PisoWiFi vendo was updated.',['router_id'=>(int)$data['router_id'],'server_ip'=>$serverIp]);$message='<div class="alert ok">Vendo updated.</div>';
      }else{
       $stmt=$this->db->prepare('INSERT INTO vendos(owner_user_id,router_id,name,base_url,server_ip,client_subnet,interface_name,password_mode,charging_enabled,eload_enabled) VALUES(?,?,?,?,?,?,?,?,?,?)');$stmt->execute([$userId,(int)$data['router_id'],$data['name'],$baseUrl,$serverIp,$subnet?:null,$data['interface_name']??null,$data['password_mode'],(int)($data['charging_enabled']??0),(int)($data['eload_enabled']??0)]);$id=(int)$this->db->lastInsertId();$this->audit('vendo.created','vendo',$id,'PisoWiFi vendo was registered.',['router_id'=>(int)$data['router_id'],'server_ip'=>$serverIp]);$message='<div class="alert ok">Vendo added.</div>';
      }
