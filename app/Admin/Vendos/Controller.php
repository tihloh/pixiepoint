...
final class Controller extends FeatureController
{
    public function index(): never
    {
        ...
        if ($this->isPost()) {
            ...
            if ($action === 'toggle_debug') {
                ...
                if ($id < 1) {
                    $message = '<div class="alert">Vendo not found.</div>';
                } else {
                    $sql = 'UPDATE vendos SET debug_enabled=? WHERE id=?';
                    ...
                    $stmt->execute($params);
                    $exists = $this->db->prepare('SELECT name FROM vendos WHERE id=?' . ($platformOwner ? '' : ' AND owner_user_id=?'));
                    ...
                    if (!$name) {
                        ...
                        $message = '<div class="alert ok">Debug ' . ($enabled ? 'enabled' : 'disabled') . ' for ' . e($name) . '.</div>';
                    }
                }
            } else {
                ...
                if ($result->fails()) {
                    ...
                    if ($baseUrl === null) {
                        $message = '<div class="alert">Vendo address must be a valid IP address, hostname, or http:// / https:// URL.</div>';
                    ...
                    } elseif ($subnet !== '' && !$this->validCidr($subnet)) {
                        $message = '<div class="alert">Client subnet must be valid CIDR, for example 10.0.3.0/24.</div>';
                        ...
                        try {
                            ...
                            if ($action === 'update') {
                                $id = max(0, (int) ($_POST['id'] ?? 0));
                                if ($id < 1) {
                                    throw new RuntimeException('Vendo not found.');
    ...
    private function normalizeBaseUrl(string $value): ?string
    {
        ...
        if (!$parts || !in_array(strtolower((string) ($parts['scheme'] ?? '')), ['http','https'], true) || trim((string) ($parts['host'] ?? '')) === '') {
            return null;
        }
