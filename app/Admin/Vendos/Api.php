...
final class Api
{
    ...
    /** Temporary diagnostics for captive-portal matching. */
    public function debugForHotspot(string $routerIdentity,string $serverIp='',string $clientIp='',string $interface=''):array
    {
        ...
        return [
            'input'=>['routerIdentity'=>$routerIdentity,'serverAddress'=>$serverIp,'clientIp'=>$clientIp,'interface'=>$interface],
            'candidateCount'=>count($result['candidates']),
            'selectedCount'=>count($result['selected']),
            'selectedIds'=>array_column($result['selected'],'id'),
            'candidates'=>array_map(function(array $v)use($serverIp,$clientIp,$interface):array{return[
                ...
                'serverMatch'=>$serverIp!==''&&(string)($v['server_ip']??'')===$serverIp,
    ...
    private function match(string $routerIdentity,string $serverIp,string $clientIp,string $interface):array
    {
        ...
        $stmt->execute([$routerIdentity]);$candidates=$stmt->fetchAll();$rows=$candidates;
