...
final class HotspotController
{
    ...
    public function portal(): never{$this->render(null);}
    public function login(): never{$this->render('login');}
    public function status(): never{$this->render('status');}
    public function logout(): never{$this->render('logout');}

    private function render(?string $state): never
    {
        $context=$this->hotspotContext();$context['state']=$state??'login';
        if($state===null&&$this->isMikroTikHandoff()){
            if(isset($_GET['link-login-only'])||isset($_GET['disconnected'])){$context['logoutUrl']='';$context['refreshUrl']='';$context['sessionTimeLeft']='';$context['bytesIn']='';$context['bytesOut']='';$context['remainBytesTotal']='';}
            $_SESSION['hotspot']=$this->sessionContext($context);header('Location: /hotspot',true,302);exit;
        }
        if($state==='login'||$state==='logout'){$context['logoutUrl']='';$context['refreshUrl']='';$context['sessionTimeLeft']='';$context['bytesIn']='';$context['bytesOut']='';$context['remainBytesTotal']='';}
        $stations=$this->api->forHotspot($context['routerIdentity'],$context['serverAddress'],$context['ip'],$context['interfaceName']);$context['passwordMode']=(string)($stations[0]['passwordMode']??'blank');$stationId=isset($stations[0]['id'])?(int)$stations[0]['id']:null;$routerId=$this->routerId($context['routerIdentity']);$resolved=(new PortalFeatureConfig($this->db))->resolve($routerId,$stationId);$theme=$this->themes->resolveByRouterIdentity($context['routerIdentity'],$stationId);$device=$this->portalDevice($context);$options=$this->vendoOptions($stations);$auth=$state==='status'?true:($state==='login'||$state==='logout'?false:$this->hasActiveHotspotSession($context));
        $_SESSION['hotspot']=$this->sessionContext($context);$feature=static fn(string $key,bool $fallback=false):bool=>isset($resolved[$key])?(bool)$resolved[$key]['enabled']:$fallback;
        ...
        $html=str_replace(['<script src="/assets/juanfi-compat.js"></script>','<script src="/assets/hotspot-login.js"></script>'],'',$html);$this->headers('text/html; charset=utf-8');echo $html;exit;
    }
    ...
    private function routerId(string $identity): int{if($identity==='')return 0;$stmt=$this->db->prepare('SELECT id FROM routers WHERE identity=? AND enabled=1 LIMIT 1');$stmt->execute([$identity]);return (int)($stmt->fetchColumn()?:0);}
    ...
    private function debugDetails(array $context): array{if(!$this->api->hasDebugTarget($context['routerIdentity']))return [];return ['raw'=>$_GET,'processed'=>$context,'matching'=>$this->api->debugForHotspot($context['routerIdentity'],$context['serverAddress'],$context['ip'],$context['interfaceName'])];}
    ...
    private function normalizeMac(string $value): string{$value=trim($value);if($value===''||str_contains($value,'$('))return '';$hex=preg_replace('/[^0-9a-fA-F]/','',$value)??'';return strlen($hex)===12?strtoupper(implode(':',str_split($hex,2))):'';}
    private function vendoOptions(array $stations): string{$options='';$selected=false;foreach($stations as $station){if(trim((string)($station['baseUrl']??''))==='')continue;$options.='<option value="'.e($station['id']).'" data-base-url="'.e($station['baseUrl']).'" data-password-mode="'.e($station['passwordMode']).'" data-charging="'.($station['chargingEnabled']?'1':'0').'" data-eload="'.($station['eloadEnabled']?'1':'0').'"'.(!$selected?' selected':'').'>'.e($station['name']).'</option>';$selected=true;}return $options;}

    private function hotspotContext(): array
    {
        $session=$this->isMikroTikHandoff()?[]:(is_array($_SESSION['hotspot']??null)?$_SESSION['hotspot']:[]);$get=static function(array $session,string ...$keys):string{foreach($keys as $key){$value=trim((string)($_GET[$key]??''));if($value!=='')return $value;}foreach($keys as $key){$value=trim((string)($session[$key]??''));if($value!=='')return $value;}return '';};
