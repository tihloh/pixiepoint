...
final class HotspotController
{
    ...
    {
        $session=is_array($_SESSION['hotspot']??null)?$_SESSION['hotspot']:[];$get=static function(array $session,string ...$keys):string{foreach($keys as $key){$value=trim((string)($_GET[$key]??''));if($value!=='')return $value;}foreach($keys as $key){$value=trim((string)($session[$key]??''));if($value!=='')return $value;}return '';};
