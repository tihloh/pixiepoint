<?php /** @var array $logs */ ?>
<div class="heading"><div><h1>Activity logs</h1><p class="muted">Review system activity for auditing and troubleshooting.</p></div></div>
<section class="panel">
    ...
    <?php foreach ($logs as $log): ?><tr>
        <td class="code"><?= e($log['action'] ?? '') ?></td>
        <td><?= e($log['actor_id'] ?? 'System') ?></td>
        <td><?= e(($log['subject_type'] ?? '—') . (($log['subject_id'] ?? null) !== null ? ' #' . $log['subject_id'] : '')) ?></td>
        <td><?= e($log['message'] ?? '—') ?></td>
        <td><?= e($log['created_at'] ?? $log['occurred_at'] ?? '—') ?></td>
    </tr><?php endforeach; ?>
    </tbody></table>
</section>
