...
final class Controller extends FeatureController
{
    ...
    public function index(): never
    {
        ...
        if($this->isPost()){$this->handlePost($access,$userId,$platformOwner,$routerId);$return=trim((string)($_POST['return_to']??''));if($return==='/admin/routers'||preg_match('~^/admin/routers/\d+(?:#stations)?$~',$return))redirect($return);$device=trim((string)($_POST['device_id']??''));redirect('/admin/vendo-gateway'.($device!==''?'?device='.rawurlencode($device):''));}
    ...
    private function handlePost(RouterAccess $access,int $userId,bool $platformOwner,int $routerId): void
    {
        ...
        try{
            ...
            if($action==='rename'){$this->bridge->rename($deviceId,(string)($_POST['vendo_name']??''));$_SESSION['admin_flash']='<div class="alert ok">Vendo renamed.</div>';return;}
            if($action==='save_config'){
                ...
                $resolved=$this->gateway->configs->resolve($deviceId);$config=$resolved['config']??[];$config['hardware']['pins']=['coin'=>$coin,'relay'=>$relay,'status_led'=>$led];$config['coin']['settle_ms']=$settle;$this->gateway->configs->setDeviceConfig($deviceId,$config);$this->gateway->commands->queue($deviceId,'config.refresh');$this->audit('vendo_gateway.device.config','vendo_gateway_device',$deviceId,'Vendo hardware configuration updated.',['router_id'=>$routerId,'station_id'=>$binding['station_id']??null]);$_SESSION['admin_flash']='<div class="alert alert-success">Vendo hardware configuration saved and queued for sync.</div>';return;
            }
            if($action==='activate')$this->gateway->devices->activate($deviceId);elseif($action==='suspend')$this->gateway->devices->suspend($deviceId);elseif($action==='revoke')$this->gateway->devices->revoke($deviceId);elseif($action==='unbind')$this->bridge->unbind($deviceId);elseif($action==='coin_enable')$this->gateway->commands->queue($deviceId,'coin.enable');elseif($action==='coin_disable')$this->gateway->commands->queue($deviceId,'coin.disable');elseif($action==='restart')$this->gateway->commands->queue($deviceId,'system.restart');elseif($action==='firmware_check')$this->gateway->commands->queue($deviceId,'firmware.check');else throw new \RuntimeException('Unknown action.');
            $this->audit('vendo_gateway.device.'.$action,'vendo_gateway_device',$deviceId,'Vendo device action executed.',['router_id'=>$routerId,'station_id'=>$binding['station_id']??null]);$_SESSION['admin_flash']='<div class="alert ok">Vendo action queued/applied.</div>';
