...
final class Controller extends FeatureController
{
    ...
    public function index(): never
    {
        ...
        foreach($this->bridge->bindings($routerId) as $binding){
            ...
            if($this->gateway){try{$resolved=$this->gateway->configs->resolve($deviceId);$config=is_array($resolved['config']??null)?$resolved['config']:[];}catch(\Throwable){}try{$firmwareStatus=$this->gateway->firmware->deviceStatus($deviceId);}catch(\Throwable){}}
    ...
    private function handlePost(RouterAccess $access,int $userId,bool $platformOwner,int $routerId): array
    {
        ...
        try{
            ...
            if($action==='save_firmware'){
                $settings=$this->gateway->firmware->normalizeSettings(['auto_check'=>isset($_POST['auto_check']),'auto_update'=>isset($_POST['auto_update']),'check_interval_hours'=>(int)($_POST['check_interval_hours']??2),'channel'=>(string)($_POST['channel']??'stable')]);
                $resolved=$this->gateway->configs->resolve($deviceId);$config=$resolved['config']??[];$config['firmware']=$settings;$this->gateway->configs->setDeviceConfig($deviceId,$config);$this->gateway->commands->queue($deviceId,'config.refresh');$this->audit('vendo_gateway.device.firmware_config','vendo_gateway_device',$deviceId,'Vendo firmware policy updated.',['router_id'=>$routerId,'station_id'=>$binding['station_id']??null]+$settings);$_SESSION['admin_flash']='<div class="alert alert-success">Firmware settings saved and queued for sync.</div>';return['ok'=>true,'message'=>'Firmware settings saved and queued for sync.','action'=>$action,'device_id'=>$deviceId,'firmware_settings'=>$settings];
            }
            if($action==='activate'){$this->gateway->devices->activate($deviceId);$message='Vendo activated.';}elseif($action==='suspend'){$this->gateway->devices->suspend($deviceId);$message='Vendo suspended.';}elseif($action==='revoke'){$this->gateway->devices->revoke($deviceId);$message='Vendo revoked.';}elseif($action==='unbind'){$this->bridge->unbind($deviceId);$message='Vendo unbound.';}elseif($action==='coin_enable'){$this->gateway->commands->queue($deviceId,'coin.enable');$message='Coin input enable queued.';}elseif($action==='coin_disable'){$this->gateway->commands->queue($deviceId,'coin.disable');$message='Coin input disable queued.';}elseif($action==='restart'){$this->gateway->commands->queue($deviceId,'system.restart');$message='Restart queued.';}elseif($action==='firmware_check'){$this->gateway->firmware->requestCheck($deviceId);$message='Firmware check queued.';}elseif($action==='firmware_update'){$this->gateway->firmware->requestUpdate($deviceId);$message='Firmware update queued. The ESP will install it when safe.';}else throw new \RuntimeException('Unknown action.');
