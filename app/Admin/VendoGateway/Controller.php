    ...
    private function handlePost(RouterAccess $access,int $userId,bool $platformOwner,int $routerId): array
    {
        ...
        try{
            ...
            if($action==='save_config'){
                $coin=(int)($_POST['coin_pin']??0);$relay=(int)($_POST['relay_pin']??0);$led=(int)($_POST['status_led_pin']??0);$settle=(int)($_POST['coin_settle_ms']??350);$platform=$this->devicePlatform($deviceId);$coinPins=$platform==='esp32'?[4,13,14,16,17,18,19,21,22,23,25,26,27,32,33]:[4,5,12,13,14];$outputPins=$platform==='esp32'?[4,13,14,16,17,18,19,21,22,23,25,26,27,32,33]:[4,5,12,13,14,16];
                if(!in_array($coin,$coinPins,true))throw new \RuntimeException('Selected coin GPIO is not supported by '.$platform.'.');if(!in_array($relay,$outputPins,true)||!in_array($led,$outputPins,true))throw new \RuntimeException('Selected relay or status LED GPIO is not supported by '.$platform.'.');if(count(array_unique([$coin,$relay,$led]))!==3)throw new \RuntimeException('Coin, relay and status LED GPIOs must be different.');if($settle<50||$settle>2000)throw new \RuntimeException('Coin settle time must be between 50 and 2000 ms.');
