...
final class Controller extends FeatureController
{
    ...
    public function index(): never
    {
        ...
        $routerId=max(0,(int)($_SESSION['pixiepoint_selected_router_id']??0));$stations=[];
        ...
        $rows=[];
        foreach($this->bridge->bindings($routerId?:null) as $binding){$device=$this->gateway?->devices->get((string)$binding['gateway_device_id']);$rows[]=['binding'=>$binding,'device'=>$device,'capabilities'=>$device&&$this->gateway?$this->gateway->devices->capabilities($device->deviceId):[]];}
