...
final class Controller extends FeatureController
{
    ...
    public function index(): never
    {
        ...
        foreach($this->bridge->bindings($routerId) as $binding){
            $deviceId=(string)$binding['gateway_device_id'];$device=$this->gateway?->devices->get($deviceId);$config=[];$reported=[];$firmwareStatus=null;
            if($this->gateway&&$device){try{$resolved=$this->gateway->configs->resolve($deviceId);$config=is_array($resolved['config']??null)?$resolved['config']:[];$config['firmware']=$this->gateway->firmware->settings($deviceId);}catch(\Throwable){}try{$firmwareStatus=$this->gateway->firmware->deviceStatus($deviceId);}catch(\Throwable){}}
    ...
    private function handlePost(RouterAccess $access,int $userId,bool $platformOwner,int $routerId): array
    {
        ...
        try{
            if(!$this->gateway)throw new \RuntimeException('Vendo Gateway is not configured.');if(!$access->canManage($routerId,$userId,$platformOwner))throw new \RuntimeException('You do not have access to manage this gateway.');
            ...
            if($action==='link_station'){
                ...
                if($deviceId===''){$this->bridge->unlinkStation($stationId);$message='Station Vendo link removed.';}else{$binding=$this->bridge->binding($deviceId);if(!$binding||(int)$binding['router_id']!==$routerId)throw new \RuntimeException('Vendo not found on this gateway.');$this->bridge->link($deviceId,$stationId);$message='Vendo linked to station.';}
                $this->audit('vendo_gateway.device.linked','station',$stationId,$message,['router_id'=>$routerId,'gateway_device_id'=>$deviceId?:null]);$_SESSION['admin_flash']='<div class="alert ok">'.e($message).'</div>';return['ok'=>true,'message'=>$message,'action'=>$action,'device_id'=>$deviceId,'reload'=>true];
            }
            $deviceId=trim((string)($_POST['device_id']??''));if($deviceId==='')throw new \RuntimeException('Device is required.');$binding=$this->bridge->binding($deviceId);if(!$binding||(int)($binding['router_id']??0)!==$routerId)throw new \RuntimeException('Vendo is not registered on the selected gateway.');
            if($action==='delete'){
                $this->db->beginTransaction();try{$this->bridge->removeInventory($deviceId);$stmt=$this->db->prepare('DELETE FROM vg_devices WHERE device_id=?');$stmt->execute([$deviceId]);$this->db->commit();}catch(\Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw$e;}
                $this->audit('vendo_gateway.device.deleted','vendo_gateway_device',$deviceId,'Vendo device was removed from PixiePoint.',['router_id'=>$routerId,'station_id'=>$binding['station_id']??null]);$_SESSION['admin_flash']='<div class="alert ok">Vendo removed. The ESP must be enrolled again before it can return.</div>';return['ok'=>true,'message'=>'Vendo removed. Re-enrollment is required.','action'=>$action,'device_id'=>$deviceId,'reload'=>true];
            }
            if($action==='unlink'){$this->bridge->unlink($deviceId);$message='Vendo unlinked from station.';$this->audit('vendo_gateway.device.unlinked','vendo_gateway_device',$deviceId,$message,['router_id'=>$routerId,'station_id'=>$binding['station_id']??null]);return['ok'=>true,'message'=>$message,'action'=>$action,'device_id'=>$deviceId,'reload'=>true];}
            if($action==='rename'){$name=trim((string)($_POST['vendo_name']??''));$this->bridge->rename($deviceId,$name);return['ok'=>true,'message'=>'Vendo renamed.','action'=>$action,'device_id'=>$deviceId,'vendo_name'=>$name?:'Vendo'];}
            if($action==='save_config'){
                ...
                $resolved=$this->gateway->configs->resolve($deviceId);$config=$resolved['config']??[];$config['hardware']['pins']['coin']=$coin;$config['hardware']['pins']['relay']=$relay;unset($config['hardware']['pins']['status_led']);$config['coin']['settle_ms']=$settle;$this->gateway->configs->setDeviceConfig($deviceId,$config);$this->gateway->commands->queue($deviceId,'config.refresh');$this->audit('vendo_gateway.device.config','vendo_gateway_device',$deviceId,'Vendo hardware configuration updated.',['router_id'=>$routerId,'station_id'=>$binding['station_id']??null,'platform'=>$platform]);return['ok'=>true,'message'=>'Hardware configuration saved and queued for sync.','action'=>$action,'device_id'=>$deviceId];
            }
            if($action==='save_firmware'){$settings=$this->gateway->firmware->saveSettings($deviceId,['auto_check'=>isset($_POST['auto_check']),'auto_update'=>isset($_POST['auto_update']),'check_interval_hours'=>(int)($_POST['check_interval_hours']??2),'channel'=>(string)($_POST['channel']??'stable')]);$this->audit('vendo_gateway.device.firmware_config','vendo_gateway_device',$deviceId,'Vendo firmware policy updated.',['router_id'=>$routerId,'station_id'=>$binding['station_id']??null]+$settings);return['ok'=>true,'message'=>'Firmware settings saved and queued for sync.','action'=>$action,'device_id'=>$deviceId,'firmware_settings'=>$settings];}
