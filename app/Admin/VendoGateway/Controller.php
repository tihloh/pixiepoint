...
final class Controller extends FeatureController
{
    ...
    public function index(): never
    {
        ...
        foreach($this->bridge->bindings($routerId) as $binding){
            ...
            if($this->gateway){try{$resolved=$this->gateway->configs->resolve($deviceId);$config=is_array($resolved['config']??null)?$resolved['config']:[];$config['firmware']=$this->gateway->firmware->settings($deviceId);}catch(\Throwable){}try{$firmwareStatus=$this->gateway->firmware->deviceStatus($deviceId);}catch(\Throwable){}}
    ...
    private function handlePost(RouterAccess $access,int $userId,bool $platformOwner,int $routerId): array
    {
        ...
        try{
            ...
            $this->audit('vendo_gateway.device.'.$action,'vendo_gateway_device',$deviceId,'Vendo device action executed.',['router_id'=>$routerId,'station_id'=>$binding['station_id']??null]);$_SESSION['admin_flash']='<div class="alert ok">'.e($message).'</div>';return['ok'=>true,'message'=>$message,'action'=>$action,'device_id'=>$deviceId,'firmware'=>$firmwareStatus??null];
