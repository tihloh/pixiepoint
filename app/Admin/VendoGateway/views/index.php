...
<div class="container-fluid py-3">
    ...
    <div class="card">
        ...
        <?php foreach($rows as $i=>$row):
            ...
            $platform=strtolower((string)($firmware['target']??$reported['hardware']['platform']??''));$platformKnown=isset($gpioProfiles[$platform]);$profile=$platformKnown?$gpioProfiles[$platform]:null;$defaults=$profile['defaults']??[];
        ...
        ?>
            <tr class="<?= $selected?'table-active':'' ?>" data-vendo-row="<?= e($deviceId) ?>">
                ...
                <td><?= $device?e(trim(($device->hardwareModel??'').' '.($device->hardwareRevision??''))):'<span class="text-danger">Missing</span>' ?><div class="small text-body-secondary text-uppercase"><?= e($platformKnown?$platform:'Target unknown') ?></div></td>
                    ...
                    <div class="col-12">
                        <div class="border rounded p-3">
                            ...
                            <form method="post" class="row g-3 align-items-end js-vendo-form"><input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="device_id" value="<?= e($deviceId) ?>"><input type="hidden" name="action" value="save_firmware">
                                <div class="col-sm-6 col-lg-3"><div class="form-check form-switch"><input class="form-check-input js-firmware-auto-check" type="checkbox" role="switch" name="auto_check" id="fw-check-<?= $i ?>" <?= ($fwConfig['auto_check']??true)?'checked':'' ?>><label class="form-check-label" for="fw-check-<?= $i ?>">ESP automatic checks</label></div></div>
...
const updateUi=(form,data)=>{const id=form.querySelector('[name=device_id]')?.value,manage=document.querySelector(`[data-vendo-manage="${CSS.escape(id||'')}"]`),row=document.querySelector(`[data-vendo-row="${CSS.escape(id||'')}"]`);if(!manage||!row)return;if(data.firmware){const fw=data.firmware,available=fw.update_available===true,latest=fw.latest_version?'v'+String(fw.latest_version).replace(/^v/i,''):'Unknown',button=manage.querySelector('[data-firmware-update]');manage.querySelector('[data-firmware-latest]').textContent=latest;manage.querySelector('[data-firmware-state]').textContent=available?'Update available':fw.update_available===false?'Up to date':'Release status unknown';manage.querySelector('[data-firmware-error]').textContent=fw.error||'';const target=manage.querySelector('[data-firmware-target]'),checked=manage.querySelector('[data-firmware-checked]');if(target)target.textContent=fw.target?'Target: '+String(fw.target).toUpperCase():'Target unknown';if(checked)checked.textContent=fw.checked_at?' · Checked '+fw.checked_at:'';row.querySelector('[data-firmware-summary]').textContent=(fw.current_version||'Unknown')+' · '+(available?latest+' available':fw.update_available===false?'Up to date':'Release status unknown');button.disabled=!available;button.textContent=available?'Update to '+latest:'Update firmware';}if(data.action==='rename'&&data.vendo_name){row.querySelector('[data-vendo-name]').textContent=data.vendo_name;form.querySelector('[name=vendo_name]').value=data.vendo_name;}if(data.action==='coin_enable')manage.querySelector('[data-coin-state]').textContent='enable queued';if(data.action==='coin_disable')manage.querySelector('[data-coin-state]').textContent='disable queued';if(['activate','suspend','revoke'].includes(data.action)){const state=data.action==='activate'?'active':data.action==='suspend'?'suspended':'revoked',cls=state==='active'?'success':state==='suspended'?'warning':'danger';row.querySelector('[data-device-state]').innerHTML=`<span class="badge text-bg-${cls}">${state}</span>`;const b=manage.querySelector('[data-state-action]');if(b&&data.action!=='revoke'){if(state==='active'){b.value='suspend';b.textContent='Suspend';b.className='btn btn-sm btn-outline-warning';}else{b.value='activate';b.textContent='Activate';b.className='btn btn-sm btn-outline-success';}}}};
