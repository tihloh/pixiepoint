...
<?php if($canManage):?><div class="modal fade" id="addVendoModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="post" class="js-vendo-form" data-reload-on-success="1">
    ...
    (() => {
        const box = document.getElementById('vendo-ajax-message'),
            ...
            message = (text, ok = true) => {
                box.innerHTML = `<div class="alert ${ok?'alert-success':'alert-danger'}">${esc(text)}</div>`;
                box.scrollIntoView({
                    behavior: 'smooth',
                    block: 'nearest'
                });
            ...
            syncPins = f => {
                ...
                s.forEach(x => [...x.options].forEach(o => {
                    o.disabled = o.value !== x.value && used.includes(o.value)
                }));
            };
        document.querySelectorAll('.js-hardware-form').forEach(f => {
            syncPins(f);
            f.querySelectorAll('.js-pin-select').forEach(s => s.addEventListener('change', () => syncPins(f)))
        });
        document.querySelectorAll('.js-firmware-auto-check').forEach(c => {
            const u = c.form?.querySelector('.js-firmware-auto-update'),
                sync = () => {
                    if (!u) return;
                    if (!c.checked) u.checked = false;
                    u.disabled = !c.checked
                };
            sync();
            c.addEventListener('change', sync)
        });
        document.querySelectorAll('[data-copy-code]').forEach(b => b.addEventListener('click', async () => {
            try {
                await navigator.clipboard.writeText(b.dataset.copyCode || '');
        ...
        if (pairingRows.length) {
            const ids = pairingRows.map(r => r.dataset.pairingId).sort(),
                deadlines = new Map(pairingRows.map(r => [r.dataset.pairingId, Date.now() + Math.max(0, Number(r.dataset.pairingRemaining || 0)) * 1000]));
            const tick = () => pairingRows.forEach(r => {
                const left = Math.max(0, Math.ceil(((deadlines.get(r.dataset.pairingId) || Date.now()) - Date.now()) / 1000)),
                    m = Math.floor(left / 60),
                    s = left % 60,
                    c = r.querySelector('[data-pairing-countdown]');
            ...
            setInterval(async () => {
                ...
                try {
                    ...
                    u.searchParams.set('pairing_status', '1');
                    ...
                    const r = await fetch(u.toString(), {
                        ...
                        d = await r.json();
                    if (d.ok) {
                        const next = (d.pairings || []).map(x => String(x.id)).sort();
        ...
        document.querySelectorAll('.js-vendo-form').forEach(f => f.addEventListener('submit', async e => {
            e.preventDefault();
            const b = e.submitter || f.querySelector('button'),
                fd = new FormData(f);
            if (b?.name && !fd.has(b.name)) fd.append(b.name, b.value);
            busy(b, true);
            try {
                ...
                message(d.message || 'Action completed.', !!d.ok);
                if (d.ok && (d.reload || f.dataset.reloadOnSuccess === '1')) {
                    setTimeout(() => location.reload(), 500);
                    return
                }
                if (d.ok && d.action === 'rename') {
                    const row = document.querySelector(`[data-vendo-row="${CSS.escape(d.device_id)}"]`);
                    row?.querySelector('[data-vendo-name]') && (row.querySelector('[data-vendo-name]').textContent = d.vendo_name)
                }
            } catch {
                message('Could not contact PixiePoint.', false)
            } finally {
                busy(b, false)
            }
        }));
