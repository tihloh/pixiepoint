...
?>
<div class="heading">
    <div><h1>Access vouchers</h1><p class="muted">Create and manage Wi-Fi access codes.</p></div>
...
<section class="panel">
    ...
    <div class="table-responsive">
        <table class="table align-middle">
            ...
            <tbody>
            <?php if (!$vouchers): ?><tr><td colspan="7" class="empty">No vouchers created.</td></tr><?php endif; ?>
            <?php foreach ($vouchers as $v): ?><tr>
                <td class="code"><strong><?= e($v['code']) ?></strong></td>
                <td><?= e($v['label'] ?: 'No label') ?></td>
                <td><?= e($v['duration_minutes']) ?> min<?php if ($v['data_limit_mb']): ?><div class="small text-body-secondary"><?= e($v['data_limit_mb']) ?> MB limit</div><?php endif; ?></td>
        ...
        <div class="modal-content">
            <form method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-grid">
                        <div class="field"><label for="voucher-code">Code</label><input id="voucher-code" name="code" placeholder="Automatic if blank"><small class="text-body-secondary">Leave blank to generate a code.</small></div>
                        <div class="field"><label for="voucher-label">Label</label><input id="voucher-label" name="label" placeholder="Optional"></div>
                        <div class="field"><label for="voucher-duration">Duration (minutes)</label><input id="voucher-duration" name="duration_minutes" type="number" min="1" value="60" required></div>
                        <div class="field"><label for="voucher-data">Data limit (MB)</label><input id="voucher-data" name="data_limit_mb" type="number" min="1" placeholder="No limit"></div>
                        <div class="field"><label for="voucher-devices">Maximum devices</label><input id="voucher-devices" name="max_devices" type="number" min="1" value="1" required></div>
                        <div class="field"><label for="voucher-uses">Maximum uses</label><input id="voucher-uses" name="max_uses" type="number" min="1" value="1" required></div>
                        <div class="field"><label for="voucher-expires">Expires at</label><input id="voucher-expires" name="expires_at" type="datetime-local"><small class="text-body-secondary">Leave blank for no expiry.</small></div>
                    </div>
                    <div class="form-check mt-2" id="voucher-enabled-wrap" hidden><input class="form-check-input" type="checkbox" name="enabled" value="1" id="voucher-enabled"><label class="form-check-label" for="voucher-enabled">Enabled</label></div>
                </div>
