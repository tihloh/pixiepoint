...
<div class="heading">
    <div>
        ...
        <div class="small text-body-secondary mt-2">Gateway: <span class="fw-semibold text-body"><?= e($router['name']) ?></span> <span class="code ms-1"><?= e($router['identity']) ?></span></div>
    </div>
    ...
    <div class="table-responsive">
        <table class="table align-middle">
            ...
            <tbody>
                ...
                <tr>
                    <td><strong class="code"><?= e($voucher['code']) ?></strong>
    ...
    <div class="table-responsive">
        <table class="table align-middle mb-0">
            ...
            <tbody><?php foreach($batches as $batch):?><tr>
                    ...
                    <td class="text-end"><a class="btn btn-sm btn-outline-primary" href="/admin/vouchers/export?batch=<?= rawurlencode($batch['batch_key']) ?>">Download script</a></td>
        ...
        <div class="modal-content">
            <form method="post">
                <div class="modal-header">
                    <h2 class="modal-title fs-5">Generate voucher batch</h2><button class="btn-close" type="button" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body"><input type="hidden" name="_csrf" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="generate">
                    <div class="row g-3">
...
$stationOptions();?></select></div>
                ...
                <div class="modal-footer"><button class="btn btn-outline-secondary" type="button" data-bs-dismiss="modal">Cancel</button><button class="button" type="submit">Generate vouchers</button></div>
            </form>
        </div>
    </div>
</div>
...
<div class="modal fade" id="voucherModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <form method="post">
                <div class="modal-header">
                    <h2 class="modal-title fs-5" id="voucherModalTitle">Create voucher</h2><button class="btn-close" type="button" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body"><input type="hidden" name="_csrf" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="create"><input type="hidden" name="id" value="0">
                    <div class="row g-3">
                        <div class="col-md-6"><label>Code</label><input name="code" placeholder="Automatic if blank"></div>
                        <div class="col-md-6"><label>Label / promo</label><input name="label"></div>
                        <div class="col-md-6"><label>Hotspot Station</label><select name="station_id"><?php
$stationOptions();?></select></div>
                        <div class="col-md-3"><label>Duration (minutes)</label><input name="duration_minutes" type="number" min="1" value="60" required></div>
                        <div class="col-md-3"><label>Data limit (MB)</label><input name="data_limit_mb" type="number" min="1"></div>
                        <div class="col-md-4"><label>Maximum devices</label><input name="max_devices" type="number" min="1" value="1" required></div>
                        <div class="col-md-4"><label>Maximum uses</label><input name="max_uses" type="number" min="1" value="1" required></div>
                        <div class="col-md-4"><label>Expires at</label><input name="expires_at" type="datetime-local"></div>
                ...
                <div class="modal-footer"><button class="btn btn-outline-secondary" type="button" data-bs-dismiss="modal">Cancel</button><button class="button" type="submit" id="voucherSubmit">Create voucher</button></div>
            </form>
        </div>
    </div>
</div>
