...
final class MikroTikAdapter implements PlatformAdapter
{
    ...
    public function transform(string $html,ThemeContext $context): string
    {
        ...
        $html=preg_replace_callback('/<input\b[^>]*\bid\s*=\s*(["\'])compat-voucher\1[^>]*>/i',static function(array $m):string{
                $tag=preg_replace('/\s+name\s*=\s*(?:"[^"]*"|\'[^\']*\'|[^\s>]+)/i','',$m[0])??$m[0];return rtrim(substr($tag,0,-1)).' name="username">';
            },$html,1)??$html;

        if($trial==='yes'&&$loginUrl!==''&&$macEsc!==''){
            $trialUrl=$loginUrl.'?dst='.($destinationEsc!==''?$destinationEsc:rawurlencode($destination)).'&username=T-'.$macEsc;
    ...
    private function chapLiteral(string $value): string
    {
        ...
        $encoded='';
        for($i=0,$length=strlen($bytes);$i<$length;$i++)$encoded.=sprintf('\\x%02x',ord($bytes[$i]));
        return "'".$encoded."'";
    }
}
