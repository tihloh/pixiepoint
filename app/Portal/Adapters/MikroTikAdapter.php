...
final class MikroTikAdapter implements PlatformAdapter
{
    ...
    public function transform(string $html,ThemeContext $context): string
    {
        ...
        if($trial==='yes'&&$loginUrl!==''&&$macEsc!==''){$trialUrl=$loginUrl.'?dst='.rawurlencode($destinationEsc!==''?$destinationEsc:$destination).'&username=T-'.$macEsc;$html=preg_replace('/<button\b[^>]*\bid\s*=\s*(["\'])pp-trial-start\1[^>]*>.*?<\/button>/is','<a id="pp-trial-start" class="btn btn-outline-success" href="'.e($trialUrl).'">Free trial</a>',$html,1)??$html;}else{$html=preg_replace('/<button\b[^>]*\bid\s*=\s*(["\'])pp-trial-start\1[^>]*>.*?<\/button>/is','',$html,1)??$html;}
    ...
    {
        $bytes=preg_replace_callback('/\\\\([0-7]{3})/',static fn(array $m):string=>chr(octdec($m[1])),$value)??$value;$encoded='';for($i=0,$length=strlen($bytes);$i<$length;$i++)$encoded.=sprintf('\\%03o',ord($bytes[$i]));return "'".$encoded."'";
    }
}
