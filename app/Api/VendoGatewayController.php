...
final class VendoGatewayController
{
    ...
    public function __construct(private PDO $db,private array $config)
    {
        ...
        if(strlen($key)>=32){$this->gateway=GatewayFactory::pdo($this->db,$key);$this->deviceAuth=new DeviceAuth(GatewayFactory::authenticator($this->db,$key));$this->bridge->register($this->gateway);}
    }
    ...
    public function pair(): never
    {
        ...
        $this->run(function() use($gateway): array{
            $result=(new PairingEndpoint($gateway->pairings))->enroll($this->body());$context=is_array($result['context']??null)?$result['context']:[];$deviceId=(string)($result['device_id']??'');$vendoId=(int)($context['vendo_id']??0);
            if($deviceId!==''&&$vendoId>0)$this->bridge->bind($deviceId,$vendoId);unset($result['context']);return $result;
        });
    }

    public function pairingAck(string $id): never
    {
        $gateway=$this->gateway();$this->run(function() use($gateway,$id): array{$payload=$this->body();$code=trim((string)($payload['setup_code']??''));return(new PairingEndpoint($gateway->pairings))->acknowledge($id,$code);});
    }
    ...
    public function config(): never{$gateway=$this->gateway();$revision=isset($_GET['revision'])?(string)$_GET['revision']:null;$this->run(fn()=>(new ConfigEndpoint($this->deviceAuth(),$gateway->configs))->pull($this->headers(),'','GET','/vendo/v1/config',$revision));}
