...
(function exposeMd5(globalScope) {
  ...
  function roundF(current, next, third, fourth, message, shift, constant) {
    return combine((next & third) | (~next & fourth), current, next, message, shift, constant);
  }

  function roundG(current, next, third, fourth, message, shift, constant) {
    return combine((next & fourth) | (third & ~fourth), current, next, message, shift, constant);
  }

  function roundH(current, next, third, fourth, message, shift, constant) {
    return combine(next ^ third ^ fourth, current, next, message, shift, constant);
  }

  function roundI(current, next, third, fourth, message, shift, constant) {
    return combine(third ^ (next | ~fourth), current, next, message, shift, constant);
  }
  ...
  globalScope.hexMD5 = function hexMD5(input) {
    ...
    for (index = 0; index < 4; index += 1) paddedInput += String.fromCharCode((lowBitLength >>> (index * 8)) & 0xff);
    for (index = 0; index < 4; index += 1) paddedInput += String.fromCharCode((highBitLength >>> (index * 8)) & 0xff);
    ...
    for (var blockOffset = 0; blockOffset < paddedInput.length; blockOffset += 64) {
      ...
      for (index = 0; index < 64; index += 1) {
        var round = index >> 4;
        var wordIndex = round === 0 ? index : round === 1 ? ((5 * index) + 1) % 16 : round === 2 ? ((3 * index) + 5) % 16 : (7 * index) % 16;
        var nextB = roundFunctions[round](accumulatorA, accumulatorB, accumulatorC, accumulatorD, words[wordIndex], shifts[round][index % 4], constants[index]);
    ...
    return wordToHex(accumulatorA) + wordToHex(accumulatorB) + wordToHex(accumulatorC) + wordToHex(accumulatorD);
  };
}(this));
