(function () {
  ...
  if (disconnectForm) {
    ...
    disconnectForm.addEventListener('submit', function (event) {
      if (window.name === 'hotspot_status') {
        if (!window.openLogout()) event.preventDefault();
      }
    });
  }
  if (loginLink) loginLink.href = session.loginUrl || '/login';
  ...
  function cancelExtension(autoCancel) {
    ...
    request('/cancelTopUp', 'POST', { voucher: session.username, mac: session.mac || '' })
      .catch(function () {})
      .then(function () { resetExtension(autoCancel ? 'No coin received. Coin slot closed.' : 'Extension cancelled.'); });
  }
