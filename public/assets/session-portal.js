(function() {
  ...
  function responseData(result) {
    ...
    try {
      return JSON.parse((result && result.body) || '{}');
    } catch (_) {
      return {};
    }
  }
  ...
  function updateExtensionCountdown(data) {
    const remainMs = number(data.remainTime),
      waitMs = number(data.waitTime),
      percent = waitMs > 0 ? Math.max(0, Math.min(100, (remainMs / waitMs) * 100)) : 100,
      seconds = Math.max(0, Math.ceil(remainMs / 1000));
  ...
  function resetExtension(message) {
    ...
    extendButton.hidden = !(vendo && session.username);
    ...
    if (message) extendStatus.textContent = message;
  }
  ...
  function handleExtensionTimeout() {
    clearTimeout(pollTimer);
    pollTimer = null;
    if (extendCoinTotal > 0) finalizeExtension(true);
    else cancelExtension(true);
  }

  function pollExtension() {
    clearTimeout(pollTimer);
    if (finalizingExtension) return;
    request('/checkCoin', 'POST', {
      voucher: session.username
    }).then(function(result) {
      const data = responseData(result),
      ...
      if (result.ok && (isTrue(data.status) || isTrue(data.success))) {
        ...
        extendStatus.textContent = extendCoinTotal > 0 ? 'Insert another coin to renew the timer, or press Done.' : 'Waiting for coin…';
        if (seconds <= 0) {
        ...
        if (extendCoinTotal > 0) finalizeExtension(true);
        else resetExtension('Coin slot was cancelled or is busy.');
        return;
  ...
  if (extendButton) extendButton.onclick = function() {
    ...
    extendVoucher.textContent = session.username || '—';
    extendStatus.textContent = 'Activating coin slot…';
    finishButton.disabled = true;
    cancelButton.disabled = false;
    startExtension(0);
  };
  if (finishButton) finishButton.onclick = function() {
