(function () {
  ...
  function rpc(path, method, data) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      window.parent.postMessage({
        type: 'pixiepoint:request', id: id, vendoId: selected && selected.id,
        path: path, method: method || 'GET', data: data || {},
      }, parentOrigin);
    });
  }
  ...
  function health() {
    ...
    rpc('/health').then(function (result) {
      ...
      setReady(true, selected.name + ' is ready'); alertMessage('');
    }).catch(function () {
  ...
  function selectVendo() {
    ...
    setReady(false, 'Checking the local vendo…'); health();
  }
  function init(data) {
  ...
  function login(voucher) {
    ...
    if (window.PIXIEPOINT_BOOTSTRAP) {
      ...
      form.password.value = chap.id ? hexMD5(chap.id + password + chap.challenge) : password;
      form.submit();
      return;
    }
  ...
  function pollCoin() {
    ...
    rpc('/checkCoin', 'POST', { voucher: activeVoucher }).then(function (result) {
      ...
      if (result.ok && (isTrue(data.status) || isTrue(data.success))) {
        displayTransaction(data); $('compat-progress').textContent = 'Coin received. Insert another coin or wait for the timer.';
        $('compat-progress-bar').style.width = '100%'; $('compat-countdown').textContent = 'Renewed';
        ...
        displayTransaction(data); var seconds = updateCountdown(data);
  ...
  function showCharging() {
    rpc('/getChargingStation', 'GET', { date: Date.now() }).then(function (result) {
      ...
      raw.split('|').filter(Boolean).forEach(function (value, index) {
        ...
        button.addEventListener('click', function () { beginTopup({ voucher: column[0], chargerPort: index, mode: 'charger' }); }); row.appendChild(button); list.appendChild(row);
      });
  ...
  function showEload() {
    var panel = $('compat-eload-panel'), products = $('compat-eload-products'); panel.hidden = false; products.textContent = 'Contacting the JuanFi e-load service…';
    rpc('/eload/rates', 'GET', { date: Date.now() }).then(function (result) {
      if (!result.ok) throw new Error('E-load rates are unavailable.');
      var data = responseData(result); if (data.raw === 'disabled') throw new Error('E-load is disabled on this vendo.');
  ...
  $('compat-topup').addEventListener('click', function () { beginTopup({ voucher: currentVoucher(), mode: 'internet' }); });
  $('compat-finish').addEventListener('click', function () { finishTopup(false); });
  $('compat-cancel').addEventListener('click', function () { cancelTopup(false); });
  ...
  $('compat-voucher-form').addEventListener('submit', function (event) { event.preventDefault(); login(currentVoucher()); });
